...
public class SemPositiveChecker implements AbsVisitor {
	...
	public void visit(AbsProgram acceptor) {
		for (AbsDecl decl : acceptor.decls.decls) {
			if(decl instanceof AbsVarDecl){
				if(((AbsVarDecl) decl).positive){
					Report.error("Only local vars can have positive", acceptor.begLine, acceptor.begColumn, acceptor.endLine, acceptor.endColumn, 1);
				}
			}
			else{
				decl.accept(this);
	...
	public void visit(AbsVarDecl acceptor) {
		if(!acceptor.positive) return;
		if(!SemDesc.getActualType(acceptor).coercesTo(new SemAtomType(SemAtomType.INT))){
			Report.error("Only integers can have positive", acceptor.begLine, acceptor.begColumn, acceptor.endLine, acceptor.endColumn, 1);
		}
	}
